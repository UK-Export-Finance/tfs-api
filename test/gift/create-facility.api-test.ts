...
const setupMocks = (options?: { expectedBusinessCalendarPayload?: { centreCode: string; startDate: string | null; exitDate: string | null } }) => {
  ...
  nock(GIFT_API_URL).persist().post(accrualScheduleNonIndexedRateUrl).reply(HttpStatus.CREATED, mockResponses.accrualSchedule);
...
describe('POST /gift/facility', () => {
  ...
  withClientAuthenticationTests({
    ...
    makeRequestWithoutAuth: (incorrectAuth?: IncorrectAuthArg) =>
      api.postWithoutAuth(apimFacilityWithoutQueueUrl, GIFT_EXAMPLES.FACILITY_CREATION_PAYLOAD, incorrectAuth?.headerName, incorrectAuth?.headerValue),
  });
  ...
  describe(`when the payload does not contain fixedFees and a ${HttpStatus.CREATED} response is returned by all GIFT endpoints`, () => {
    it(`should return a ${HttpStatus.CREATED} response with a facility and all created entities`, async () => {
      // Arrange
      setupMocks();

      // Act
      const { status, body } = await api.post(apimFacilityWithoutQueueUrl, GIFT_EXAMPLES.FACILITY_CREATION_PAYLOAD_NO_FIXED_FEES);
  ...
  describe(`when the payload does not contain repaymentProfiles and a ${HttpStatus.CREATED} response is returned by all GIFT endpoints`, () => {
    it(`should return a ${HttpStatus.CREATED} response with a facility and all created entities`, async () => {
      // Arrange
      setupMocks();

      // Act
      const { status, body } = await api.post(apimFacilityWithoutQueueUrl, GIFT_EXAMPLES.FACILITY_CREATION_PAYLOAD_NO_REPAYMENT_PROFILES);
