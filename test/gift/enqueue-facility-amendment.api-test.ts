...
describe('POST /gift/facility/:facilityId/amendment', () => {
  ...
  withClientAuthenticationTests({
    ...
    makeRequestWithoutAuth: (incorrectAuth?: IncorrectAuthArg) => {
      const req = request(app.getHttpServer()).post(apimFacilityAmendmentUrl).send(GIFT_EXAMPLES.FACILITY_AMENDMENT_REQUEST_PAYLOAD);
      if (incorrectAuth) {
  ...
  describe('when the payload is valid', () => {
    it('should return 202', async () => {
      const { status } = await request(app.getHttpServer())
        .post(apimFacilityAmendmentUrl)
        .send(GIFT_EXAMPLES.FACILITY_AMENDMENT_REQUEST_PAYLOAD)
