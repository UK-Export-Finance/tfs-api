import { UKEFID } from '../ukef-id.constant';

const CURRENCY_VALIDATION = {
  MIN_LENGTH: 3,
  MAX_LENGTH: 3,
};

/**
 * Date string validation
 * E.g: 2025-01-02
 */
const DATE_STRING = {
  MIN_LENGTH: 10,
  MAX_LENGTH: 10,
};

const FEE_TYPE_CODE = {
  MIN_LENGTH: 3,
  MAX_LENGTH: 3,
};

/**
 * Maximum monetary amount.
 * This is used for any monetary "amount" field.
 */
const MAX_MONETARY_AMOUNT = Number.MAX_SAFE_INTEGER;

const GENERIC_STRING_RULE = {
  MIN_LENGTH: 1,
  MAX_LENGTH: 100,
};

export const VALIDATION = {
  ACCRUAL_SCHEDULE: {
    ACCRUAL_DAY_BASIS_CODE: GENERIC_STRING_RULE,
    ACCRUAL_FREQUENCY_CODE: GENERIC_STRING_RULE,
    ACCRUAL_SCHEDULE_TYPE_CODE: GENERIC_STRING_RULE,
    ACCRUAL_SCHEDULE_INDEX_RATE_CODE: { MIN_LENGTH: 6, MAX_LENGTH: 10 },
    ADDITIONAL_RATE: { MIN: 0 }, // no max validation in GIFT.
    BASE_RATE: {}, // no min/max validation in GIFT.
    SPREAD_RATE: { MIN: 0 }, // no max validation in GIFT.
  },
  CURRENCY: CURRENCY_VALIDATION,
  DATE_STRING,
  FACILITY: {
    AMENDMENT_TYPE: { MIN_LENGTH: 1, MAX_LENGTH: 40 },
    AMENDMENT: {
      AMOUNT: {
        MIN: 1,
        MAX: MAX_MONETARY_AMOUNT,
        MAX_DECIMAL_PLACES: 2,
      },
    },
    OVERVIEW: {
      CREDIT_TYPE: { MIN_LENGTH: 1, MAX_LENGTH: 120 },
      CURRENCY: CURRENCY_VALIDATION,
      FACILITY_AMOUNT: { MIN: 1, MAX: MAX_MONETARY_AMOUNT },
      FACILITY_ID: UKEFID.VALIDATION,
      FACILITY_NAME: { MIN_LENGTH: 1, MAX_LENGTH: 35 },
      OBLIGOR_URN: { MIN_LENGTH: 8, MAX_LENGTH: 8 },
      PRODUCT_TYPE_CODE: { MIN_LENGTH: 3, MAX_LENGTH: 30 },
      REPAYMENT_TYPE: GENERIC_STRING_RULE,
    },
  },
  COUNTERPARTY: {
    COUNTERPARTY_URN: { MIN_LENGTH: 8, MAX_LENGTH: 8 },
    ROLE_CODE: {
      MIN_LENGTH: 1,
      MAX_LENGTH: 50,
    },
    SHARE_PERCENTAGE: { MIN: 0.1, MAX: 100 },
  },
  FEE_TYPE_CODE,
  FIXED_FEE: {
    AMOUNT_DUE: { MIN: 1, MAX: MAX_MONETARY_AMOUNT },
    CURRENCY: CURRENCY_VALIDATION,
    DESCRIPTION: { MIN_LENGTH: 1, MAX_LENGTH: 128 },
    FEE_TYPE_CODE,
  },
  MAX_MONETARY_AMOUNT,
  OBLIGATION: {
    CURRENCY: CURRENCY_VALIDATION,
    OBLIGATION_AMOUNT: { MIN: 1, MAX: MAX_MONETARY_AMOUNT },
    OBLIGATION_SUBTYPE_CODE: { MIN_LENGTH: 1, MAX_LENGTH: 30 },
    LINKED_REPAYMENT_PROFILE_ID: { MIN: 1, MAX: MAX_MONETARY_AMOUNT },
    REPAYMENT_TYPE: GENERIC_STRING_RULE,
  },
  REPAYMENT_PROFILE: {
    NAME: { MIN_LENGTH: 1, MAX_LENGTH: 120 },
    ALLOCATION: {
      AMOUNT: { MIN: 0, MAX: MAX_MONETARY_AMOUNT },
    },
  },
  RISK_DETAILS: {
    ACCOUNT: { MIN_LENGTH: 1, MAX_LENGTH: 10 },
    DEAL_ID: UKEFID.VALIDATION,
    FACILITY_CATEGORY_CODE: GENERIC_STRING_RULE,
    FACILITY_CREDIT_RATING: GENERIC_STRING_RULE,
    RISK_STATUS: { MIN_LENGTH: 1, MAX_LENGTH: 50 },
    UKEF_INDUSTRY_CODE: { MIN_LENGTH: 4, MAX_LENGTH: 4 },
  },
};
