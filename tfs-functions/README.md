...
The call is retried up to `GIFT_MAX_NUMBER_OF_RETRIES` times (also set manually in
host.json); after that, a Halo support ticket is automatically raised via
`src/utils/create-halo-ticket.ts`
...
`GiftFacilityAmendmentRequestDto` shape as Base64 and post it directly:
...
- Messages must be Base64-encoded JSON. The `GiftQueueService` in `tfs-api`
   handles this automatically when using the `/facility/queue` endpoint.
...
When a GIFT facility creation or amendment request fails, the function raises a
support ticket in Halo PSA before rethrowing the error. The ticket includes the
facility ID, the full original payload, and the error message.

Authentication uses OAuth2 client credentials (`HALO_AUTH_CLIENT_ID` / `HALO_CLIENT_SECRET`) against the `HALO_BASE_URL` tenant.

To test locally, you need access to the Halo Test environment and must set these
authentication credentials accordingly. The variables below will make more sense
when cross-referenced with the Halo UI.
...
To trigger a Halo ticket during local testing, put a message onto the queue that
will cause the GIFT facility creation or amendment to fail (e.g. an invalid
payload). The function will attempt the GIFT call, fail, and then call Halo
before moving the message to the poison queue after the fifth retry.
...
|Private endpoint|`pep-apim-<env>-<version>-st-queue`|Privately exposes the queue storage endpoint in the VNet|
...
|`id-apim-functions-*`|Storage Blob Data Owner|Storage account|Required by the Functions webjobs runtime with managed identity and host locks/heartbeats|
...
|`APIM_TFS_KEY`|`x-api-key` tfs-api auth header|APIM subscription key header|
|`APIM_TFS_VALUE`|Must match the local tfs-api `API_KEY` value|APIM subscription key, stored as a GitHub secret and injected as a container app secret|

Locally, the functions app calls tfs-api directly and authenticates with its
`x-api-key` header. In deployed environments, all traffic is routed through APIM
using an APIM subscription key.
...
The `__accountName` / `__credential` / `__clientId` convention is the Azure
Functions v4 passwordless storage binding format. The Functions runtime uses
this to acquire tokens via managed identity rather than a connection string.
