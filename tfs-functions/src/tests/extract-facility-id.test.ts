...
describe('extractFacilityId', () => {
  describe('when messageType is facility-amendment', () => {
    it('returns the facilityId from the message', () => {
      const item = { messageType: GIFT_QUEUE_MESSAGE_TYPE.FACILITY_AMENDMENT, facilityId: TEST_FACILITY_ID, payload: {} };

      expect(extractFacilityId(item)).toBe(TEST_FACILITY_ID);
    });
  ...
  describe('when messageType is facility-creation', () => {
    it('returns the facilityId from the payload overview', () => {
      const item = { messageType: GIFT_QUEUE_MESSAGE_TYPE.FACILITY_CREATION, payload: { overview: { facilityId: TEST_FACILITY_ID } } };
